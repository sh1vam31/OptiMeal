...
export default function OnboardingPage({
  ...
  // Toggle persona filter without auto-selecting any dishes
  ...
  return (
    <div className="min-h-screen bg-[#0D0F12] text-white flex flex-col justify-between pb-28 space-y-8 max-w-7xl mx-auto px-4 py-6">
      
      {/* Clean Brand Header */}
      <div className="flex items-center space-x-2.5 pt-2">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-rose-600 to-amber-500 p-2 shadow-lg shadow-rose-600/20 flex items-center justify-center text-white font-black">
      ...
      <div className="space-y-3 max-w-3xl">
        <span className="text-xs font-mono font-black uppercase text-rose-500 tracking-widest px-3 py-1 rounded-full bg-rose-500/10 border border-rose-500/20 inline-block">
          AI FOOD RECOMMENDER
        </span>
        ...
        <p className="text-xs md:text-sm text-gray-300 leading-relaxed font-normal">
          Pick at least <strong className="text-rose-400 font-extrabold">five dishes</strong> below. IntentEats blends your picks with your budget and delivery speed to build your personalized menu.
        </p>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-xs font-mono font-extrabold uppercase tracking-wider text-gray-400">
          ...
          {selectedPersona && (
            <button
              ...
              className="px-3 py-1 rounded-xl bg-rose-500/15 hover:bg-rose-500/25 text-rose-300 border border-rose-500/30 text-xs font-bold transition-all flex items-center gap-1.5 shadow-sm cursor-pointer"
            >
            ...
            return (
              <button
                ...
                className={`p-5 rounded-2xl text-left transition-all border flex flex-col justify-between space-y-3 relative overflow-hidden group cursor-pointer ${
                  isSelected
                    ? 'bg-gradient-to-b from-rose-900/60 to-[#141820] border-rose-500 shadow-xl shadow-rose-600/20 scale-[1.02]'
                    : 'bg-[#141820] border-white/10 hover:border-white/20 hover:bg-[#1C2128]'
                }`}
              >
                <div className="text-3xl">{p.icon}</div>
                <div>
                  <h3 className="font-extrabold text-sm text-white group-hover:text-rose-300 transition-colors">
                    {p.title}
                  </h3>
                ...
                {isSelected ? (
                  <span className="text-[10px] font-mono font-black text-rose-400 uppercase tracking-wider block pt-1">
                    ACTIVE FILTER ✓
                  </span>
                ) : (
                  <span className="text-[10px] font-mono text-gray-500 block pt-1 group-hover:text-gray-300">
                    Click to filter →
                  </span>
                )}
              </button>
          ...
          {displayDishes.slice(0, 120).map((item) => {
            ...
            return (
              <div
                ...
                className={`group rounded-2xl overflow-hidden bg-[#141820] border transition-all cursor-pointer relative flex flex-col justify-between ${
                  isPicked
                    ? 'border-rose-500 ring-2 ring-rose-500/50 scale-[1.03]'
              ...
              >
                  ...
                  <button
                    ...
                    className={`absolute top-2 right-2 w-7 h-7 rounded-full flex items-center justify-center transition-all border ${
                      isPicked
                        ...
                        : 'bg-black/60 text-white border-white/30 hover:bg-white hover:text-slate-950'
                    }`}
                  >
                    {isPicked ? <Check className="w-4 h-4" /> : <Plus className="w-4 h-4" />}
                  </button>

                  {/* Color Symbol Dot (Green 🟢 / Red 🔴) */}
                ...
                <div className="p-2.5 space-y-0.5">
                  <h3 className="text-xs font-bold text-gray-200 group-hover:text-white line-clamp-2 min-h-[2rem] leading-tight">
                    {item.name}
                  </h3>
                  <span className="text-[10px] text-emerald-400 font-mono font-bold block">₹{item.price}</span>
                </div>
              </div>
            );
          })}
      ...
      {/* Sticky Bottom Bar */}
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-[#0D0F12]/95 backdrop-blur-md border-t border-white/10 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <button
            ...
            className={`px-8 py-3.5 rounded-2xl font-black text-sm transition-all flex items-center gap-2 ${
              canProceed
                ? 'bg-rose-600 hover:bg-rose-500 text-white shadow-xl shadow-rose-600/30 cursor-pointer active:scale-95'
          ...
          >
            ...
            {canProceed ? <ArrowRight className="w-4 h-4" /> : <Lock className="w-4 h-4 text-gray-500" />}
          </button>
        </div>
      </div>
    </div>
